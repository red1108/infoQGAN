# 1. seed는 모두 동일한 분포
# 2. Discriminator에는 latent vector을 넣되, Mine 입력으로는 Decode을 거쳐 28x28이미지를 넣는다.
# Standard Libraries
import math
import os
import pickle
...
generator = QGenerator.QGAN4(n_qubits, output_qubits, n_layers, generator_initial_params, dev)
discriminator = Discriminator.LinearDiscriminator(input_dim = latent_dim, hidden_size=100) # 50 --> 25 변경
mine = MINE.LinearMine(code_dim=code_qubits, output_dim=28*28, size=50)
...
def generator_train_step(generator_seed, coeff, use_mine = False):
    ...
    if use_mine:
        # mine 학습은 decoder을 거쳐서 진행.
        with torch.no_grad():
            reconstructed_images = autoencoder.decoder(generator_output)  # (BATCH_SIZE, 1, 28, 28)
        flattened_images = reconstructed_images.view(reconstructed_images.size(0), -1) # (BATCH_SIZE, 784)
        pred_xy = mine(code_input, flattened_images)
        code_input_shuffle = code_input[torch.randperm(BATCH_SIZE)]
        pred_x_y = mine(code_input_shuffle, flattened_images)
...
convert_py_to_html('new_mnist_train.py', os.path.join(save_dir, 'new_mnist_train.html'))
# ==========================================================
